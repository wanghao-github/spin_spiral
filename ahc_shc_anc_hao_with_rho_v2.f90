program ahc_shc_anc_hao
    ...
    real(kind=dp) :: kpoints(3)
    real(kind=dp) :: scale, Beta_fake, mu
    ...
    real(kind=dp) :: bfield(3)
    integer :: rvecnum, num_lines, length
    integer, allocatable :: irvec(:,:)
    real, allocatable :: kpts(:,:), crvec(:,:)
    real(kind=dp) :: kder, amat(3,3)
    real(kind=dp) :: volume, cross_vec(3), mag_field_x, mag_field_y, mag_field_z
    real(kind=dp) :: bohrincm, condq, mag_strength, mag_theta, mag_phi
    integer :: maxdim, num_kpts
    real(kind=dp) :: minenerg, maxenerg, gamma, kbT
    ...
    twopi = 2.0_dp * 3.141592653589793d0

    bohrincm = 0.5291772_dp * 1.0e-8_dp
    condq = 38.7405_dp * 1.0e-6_dp

    if (irank == 0) then
        open(unit=300, file='ahe_inp', status='old', action='read')
        read(300,*) amat(1, :)
        read(300,*) amat(2, :)
        read(300,*) amat(3, :)
        ...
        read(300,*) (mag_wann_orbs_index(j), j=1, mag_wann_num)
        close(300)
    ...
    if (irank == 0) then
        open(unit=200, file='hopping.1', status='old', action='read')
        num_lines = 0
        num_wann = 0
        do
            read(200, *, iostat=info) ix, iy, iz, band1, band2, rdum, idum
            if (info /= 0) exit
            num_lines = num_lines + 1
            num_wann = max(num_wann, max(band1, band2))
        end do
        ...
        do ii = 1, rvecnum
            crvec(:, ii) = amat(1, :) * irvec(1, ii) + &
                           amat(2, :) * irvec(2, ii) + &
                           amat(3, :) * irvec(3, ii)
        end do

        allocate(rspauli(1:num_wann, 1:num_wann, 3, rvecnum))
        open(unit=400, file='./rspauli.1', status='old', action='read')
        num_lines = 0
        do
            read(400, *, iostat=info) ix, iy, iz, band1, band2, dir, rdum, idum
            if (info /= 0) exit
            num_lines = num_lines + 1
        ...
        open(unit=14, file='nrpts_inp', status='old', action='read')
        do j = 1, rvecnum / 15
    ...
    volume = cross_vec(1) * amat(3, 1) + &
             cross_vec(2) * amat(3, 2) + &
             cross_vec(3) * amat(3, 3)
    ...
    do ik = 1 + irank, knv3, isize
        ...
        do ii = 1, rvecnum
            ...
            iy = irvec(2, ii)
            iz = irvec(3, ii)
            phas = iy * kpoints(2) + iz * kpoints(3) + ix * kpoints(1)
            phas = phas * twopi
            fac = cmplx(cos(phas), sin(phas), kind=dp)
        ...
        do ii = 1, rvecnum
            ...
            phas = iy * kpoints(2) + iz * kpoints(3) + ix * kpoints(1)
            phas = phas * twopi
            fac = cmplx(-sin(phas), cos(phas), kind=dp)
        ...
        do dir = 1, 3
            spinmomentum(:,:,dir) = (MATMUL(pauli_matrix(:,:,3), momentum(:,:,dir)) + &
                                       MATMUL(momentum(:,:,dir), pauli_matrix(:,:,3))) / 2.0_dp
        end do
        ...
        do step = 1, num_steps
            fermienergy(step) = fermi_min + (fermi_max - fermi_min) * real(step, kind=dp) / real(num_steps, kind=dp)
            mu = fermienergy(step)
        ...
        do n = 23, num_wann
            do m = 1, 22
                if (abs(eigvals(m) - eigvals(n)) > 1.0e-8_dp) then
                    Omega_z_shc(m) = Omega_z_shc(m) - aimag(momentum2(m,n,1) * conjg(spinmomentum2(m,n,2))) / (eigvals(m) - eigvals(n))**2
                    Omega_y_shc(m) = Omega_y_shc(m) - aimag(momentum2(m,n,1) * conjg(spinmomentum2(m,n,3))) / (eigvals(m) - eigvals(n))**2
                    Omega_x_shc(m) = Omega_x_shc(m) - aimag(momentum2(m,n,2) * conjg(spinmomentum2(m,n,3))) / (eigvals(m) - eigvals(n))**2
                end if
            end do ! n
        end do ! m
    ...
    ! 聚合所有进程的 rho 矩阵到 rho_mpi（复数）
    call MPI_REDUCE(rho, rho_mpi, size(rho), MPI_DOUBLE_COMPLEX, MPI_SUM, 0, mpi_comm_world, ierr)

    ! 聚合 sigma_tensor_ahc_mpi 和 sigma_tensor_shc_mpi 到根进程
    call MPI_REDUCE(sigma_tensor_ahc_mpi, sigma_tensor_ahc_mpi2, num_steps_tot * 3, MPI_DOUBLE_PRECISION, MPI_SUM, 0, mpi_comm_world, ierr)
    call MPI_REDUCE(sigma_tensor_shc_mpi, sigma_tensor_shc_mpi2, num_steps_tot * 3, MPI_DOUBLE_PRECISION, MPI_SUM, 0, mpi_comm_world, ierr)
    ...
    if (irank == 0) then
        ...
        ! 调用 pauli_block_all 处理 rho_mpi 的实部
        write(*,*) "rho_mpi = ", rho_mpi
        call pauli_block_all(real(rho_mpi, kind=dp), num_wann, pauli_result)

        ! 输出结果
        open(unit=123, file='output_ahe_condquant', status='unknown', action='write', recl=10000)
        do step = 1, num_steps
            write(123,*) "fermienergy=", fermienergy(step)
            write(123,*) "occupation=", occupation(step)
            write(123,*) "conductivity=", conductivity_ahe(step) / condq * amat(3,3) * 1.0e-8_dp
            write(123,*) "conductivity13=", conductivity13_ahe(step) / condq * amat(3,3) * 1.0e-8_dp
            write(123,*) "conductivity23=", conductivity23_ahe(step) / condq * amat(3,3) * 1.0e-8_dp
            write(123,*) "************************************"
        end do
        close(123)

        open(unit=321, file='output_she_condquant', status='unknown', action='write', recl=10000)
        do step = 1, num_steps
            write(321,*) "fermienergy=", fermienergy(step)
            write(321,*) "occupation=", occupation(step)
            write(321,*) "conductivity=", conductivity_she(step) / condq * amat(3,3) * 1.0e-8_dp
            write(321,*) "conductivity13=", conductivity13_she(step) / condq * amat(3,3) * 1.0e-8_dp
            write(321,*) "conductivity23=", conductivity23_she(step) / condq * amat(3,3) * 1.0e-8_dp
            write(321,*) "************************************"
        end do
        close(321)

        open(unit=458, file='charge_on_each_atom', status='unknown', action='write', recl=10000)
        do m = 1, 4
            write(458,*) "m= ", m
...
function fermi(omega, Beta_fake) result(value)
    ...
    real(kind=dp) :: value
    if (Beta_fake * omega .ge. 20.0_dp) then
        value = 0.0_dp
    elseif (Beta_fake * omega .le. -20.0_dp) then
        value = 1.0_dp
    else
...
subroutine now(time_now)
    ...
    time_now = time_new(3) * 24.0_dp * 3600.0_dp + &
               time_new(5) * 3600.0_dp + &
               time_new(6) * 60.0_dp + &
               time_new(7) + time_new(8) / 1000.0_dp
    return
end subroutine now
